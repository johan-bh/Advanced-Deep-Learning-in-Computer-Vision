...
class Attention(nn.Module):
    def __init__(self, embed_dim, num_heads):
        ...
        self.attention = None  # Add this line to store attention weights
    ...
    def forward(self, x):
        ...
        self.attention = attention  # Store the attention weights
...
class ViT(nn.Module):
    def __init__(self, image_size, channels, patch_size, embed_dim, num_heads, num_layers,
        ...
        # Create custom Rearrange layer that stores patch size
        class PatchRearrange(nn.Module):
            def __init__(self, p1, p2):
                super().__init__()
                self.p1 = p1
                self.p2 = p2
                self.patch_height = p1
                self.patch_width = p2
        ...
        self.patch_embedding = nn.Sequential(
            PatchRearrange(patch_h, patch_w),
            nn.LayerNorm(patch_dim),
            nn.Linear(patch_dim, embed_dim)
        )
        self.to_patch_embedding = self.patch_embedding
