...
def image_to_patches(image, patch_size, image_grid=True):
    ...
    nh = H // patch_h  # number of patches in height
    nw = W // patch_w  # number of patches in width
    ...
    if image_grid:
        ...
        """
        patches = rearrange(image, 'b c (nh ph) (nw pw) -> b (nh nw) c ph pw', 
                          ph=patch_h, pw=patch_w, nh=nh, nw=nw)
        patches = rearrange(patches, 'b n c ph pw -> b n (c ph pw)', 
